...
Account deletion purges the deleted repo's mirror; reversible states retain their raw mirror for reactivation, as before. Identity events update the handle only. A `#sync` event purges the mirrored repo, then the authoritative replacement commits that follow repopulate it in stream order. The lifecycle policy lives in `internal/discoveringest`.
