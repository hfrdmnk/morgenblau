...
- Session caches follow the `lib/library-cache.ts` shape (module-level entry, TTL, `read*`/`write*` pair). They bridge a page across mount/unmount within one session; React state stays the source of truth.
