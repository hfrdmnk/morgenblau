...
func TestReconcileShares_DuplicateSidecarNewestWinsOlderDeleted(t *testing.T) {
	// Two sidecars for one document (a sync/PATCH race): the newest edit wins, the older duplicate is deleted from the PDS.
	store := newFakeStore()
...
func TestReconcileShares_RekeyedCanonicalDeletesBeforeUpsert(t *testing.T) {
	// The canonical recommend for docA changed rkey (delete+recreate elsewhere); the stale row still holds (did, document), so the partial unique index forces delete before upsert.
	store := newFakeStore()
	store.shares["did:plc:alice"] = map[string]db.ListUserSharesForSyncRow{
...
func TestReconcileShares_BareRecommendBackfillsItemURL(t *testing.T) {
	// A recommend with no comment sidecar: reconcile fills item_url from the cached entry so the display fallback survives the entry's later deletion.
	store := newFakeStore()
...
// assertDeleteBeforeUpsert fails unless delRkey's delete precedes upRkey's upsert in the op-log, the ordering SQLite's unique constraints require.
