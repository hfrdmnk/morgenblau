...
// TestCachedReaderFollowCrawler_WiresReaderWriterAndTTL is the thin instantiation check: the
// cache-logic itself (TTL math, degrade posture) lives in cachedFetch and is covered by its own tests.
func TestCachedReaderFollowCrawler_WiresReaderWriterAndTTL(t *testing.T) {
	...
	// The ttl passed to NewCachedReaderFollowCrawler is what gets honored, not some hardcoded value.
