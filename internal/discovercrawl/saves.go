...
func (c *Client) CrawlSaves(ctx context.Context, did syntax.DID) ([]Save, error) {
	...
	byCollection := make(map[string][]recordEntry, len(saveShapes))
	for _, shape := range saveShapes {
		...
		byCollection[shape.collection] = records
	}
	return DecodeSaves(byCollection), nil
}

func appendSaves(out *[]Save, seen map[string]struct{}, records []recordEntry, shape saveShape) {
	for _, r := range records {
		save, ok := decodeSave(shape, r)
		if !ok {
			continue
		}
		if _, dup := seen[save.ItemURL]; dup {
			continue
		}
		seen[save.ItemURL] = struct{}{}
		*out = append(*out, save)
	}
}

func decodeSave(shape saveShape, r recordEntry) (Save, bool) {
	...
	return Save{Kind: shape.kind, ItemURL: itemURL, FeedURL: feedURL, CreatedAt: createdAt}, true
}
