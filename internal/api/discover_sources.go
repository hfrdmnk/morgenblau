...
// discoverCrawlFanoutLimit bounds concurrent per-candidate PDS crawls (shared by DiscoverSourcesHandler, DiscoverPeopleHandler, and LibraryNetworkSharesHandler) so a cold cache with hundreds of candidates can't flood SQLite's single writer connection or blow the server's 30s WriteTimeout.
...
// SubscriptionCrawler fetches a followed person's feed subscriptions.
...
// DiscoverSourceTitleBackfillReader looks up the network-wide trending table for one source key's title/siteUrl, used to backfill reaction-only rss candidates that addSignal never gave a title. SPEC <discovery>.
...
func DiscoverSourcesHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// subjectDIDs is sorted (not map order) so the goroutine fan-out below reassembles results deterministically regardless of completion order.
		...
		// slots[i] is written only by goroutine i, so no lock is needed; folding
		// into candidates happens single-threaded afterward in subjectDIDs
		// order, making output deterministic regardless of completion order.
		slots := make([][]pendingSourceSignal, len(subjectDIDs))
		g, gctx := errgroup.WithContext(r.Context())
		g.SetLimit(discoverCrawlFanoutLimit)
		for i, subjectDID := range subjectDIDs {
			i, subjectDID := i, subjectDID
			g.Go(func() error {
				slots[i] = crawlSourceCandidate(gctx, subjectDID, crawler, authored, shares, entries)
		...
		for i, subjectDID := range subjectDIDs {
			...
			for _, ps := range slots[i] {
				addSignal(candidates, ps.key, ps.kind, ps.title, ps.siteURL, follower, ps.signalKind, ps.at)
			}
		}
		...
		if foreign, err := ownForeign.CrawlOwnForeignSubscriptions(r.Context(), sess.Data.AccountDID); err != nil {
		...
		backfillMissingTitles(r.Context(), titleBackfill, candidates)
		...
		hiddenKeys, err := hides.ListActiveDiscoverHides(r.Context(), db.ListActiveDiscoverHidesParams{
			Did:         didStr,
		...
		if !cursor.Personal.Done {
			personalPage = discoverrank.RankPage(list, excluded, discoverPageSize, cursor.Seed, rankingNow, cursor.Personal.After)
		}
		...
		trendingRows, err := trending.ListDiscoverTrendingSignalsAboveBar(r.Context(), discoverrank.MinDistinctRepos)
		if err != nil {
		...
		if langRows, err := languages.ListFeedLanguages(r.Context()); err != nil {
			...
			readerLangs := discoverlang.ReaderLanguages(subLangs, discoverlang.ParseAcceptLanguage(r.Header.Get("Accept-Language")))
		...
		if !cursor.Trending.Done {
			trendingPage = discoverrank.RankTrendingPage(trendingCandidates, excludedForTrendingOnly, discoverPageSize, cursor.Seed, rankingNow, cursor.Trending.After)
		}
...
// backfillMissingTitles fills title/siteUrl for reaction-only rss candidates from the network-wide trending table, since a share/save signal carries neither (see resolveReactionKey). Not gated by the trending quality bar: a title only needs one contributing repo. A lookup miss or error skips that candidate; it must never fail the request.
func backfillMissingTitles(ctx context.Context, reader DiscoverSourceTitleBackfillReader, candidates map[string]*discoverrank.Candidate) {
	for key, c := range candidates {
		...
		row, err := reader.GetDiscoverTrendingSignalTitle(ctx, key)
		if err != nil {
			if !errors.Is(err, sql.ErrNoRows) {
				slog.Debug("/api/discover/sources: title backfill lookup failed", "key", key, "err", err)
			}
			continue
		}
...
// pendingSourceSignal holds one candidate's crawled signal for the caller to fold in after all goroutines finish; addSignal itself isn't safe for concurrent calls since it mutates a shared map.
type pendingSourceSignal struct {
	key, kind, title, siteURL string
	signalKind                discoverrank.SignalKind
	at                        time.Time
}

// crawlSourceCandidate runs one candidate's four signal crawls; a malformed DID or single crawl failure only degrades that signal, never returned as an error, so one candidate can't abort the others.
func crawlSourceCandidate(
	ctx context.Context,
	subjectDID string,
	crawler SubscriptionCrawler,
	authored AuthoredPublicationCrawler,
	...
	followedDID, err := syntax.ParseDID(subjectDID)
	if err != nil {
		slog.Warn("/api/discover/sources: malformed followed did", "did", subjectDID, "err", err)
		return nil
	}

	var signals []pendingSourceSignal

	if found, err := crawler.FetchSubscriptions(ctx, followedDID); err != nil {
		slog.Warn("/api/discover/sources: subscription crawl failed", "did", subjectDID, "err", err)
	} else {
		for _, s := range found {
			signals = append(signals, pendingSourceSignal{key: s.Key, kind: s.Kind, title: s.Title, siteURL: s.SiteURL, signalKind: discoverrank.SignalSubscribe, at: parseDiscoverTime(s.CreatedAt)})
		}
	}

	if pubs, err := authored.FetchAuthoredPublications(ctx, followedDID); err != nil {
		slog.Warn("/api/discover/sources: authored-publication crawl failed", "did", subjectDID, "err", err)
	} else {
		for _, p := range pubs {
			signals = append(signals, pendingSourceSignal{key: p.Key, kind: p.Kind, title: p.Title, siteURL: p.SiteURL, signalKind: discoverrank.SignalAuthor, at: parseDiscoverTime(p.LastPublishedAt)})
		}
	}

	if found, err := shares.FetchShares(ctx, followedDID); err != nil {
		slog.Warn("/api/discover/sources: share crawl failed", "did", subjectDID, "err", err)
	} else {
		for _, sh := range found {
			...
			signals = append(signals, pendingSourceSignal{key: key, kind: kindForReactionKey(key), signalKind: discoverrank.SignalShare, at: parseDiscoverTime(sh.CreatedAt)})
		}
	}
